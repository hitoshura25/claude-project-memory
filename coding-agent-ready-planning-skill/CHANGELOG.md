...
| 2026-03-22 (Chat 9) | `references/stacks/infra.md` | **Fix**: Docker research verification checklist — 4 verification gates (initialization, CMD correctness, user/permissions, compose health) that must pass before writing Dockerfile; removed unnecessary Dockerfile comment requirement |
| 2026-03-23 (Chat 9) | `SKILL.md` (agent-ready-plans) Step 5 | **Fix**: Grounding rule extended to Interface Contracts — task doc code blocks must be copied from the validated stub, not the plan. Explicit prohibition: comments masquerading as defaults (e.g., `# default: "value"`) must be replaced with actual code (`= "value"`) |
| 2026-03-23 (Chat 9) | `implementation-planning/references/plan-format.md` | **Fix**: Interface Contract "Include" list — actual default values as code, not comments. Explicit example of wrong vs right |
| 2026-03-24 (Chat 10) | All skill files | **Major refactor**: implementation-planning absorbs scaffold/validation; agent-ready-plans shrinks to packaging only. Reference files and scripts reorganized. writing-guide.md split into test-writing-guide.md + task-doc-guide.md |
| 2026-03-24 (Chat 10/T44) | `implementation-planning/scripts/validate-stubs.sh` | **New**: Automated Layer 2 validation script — runs each test file individually, parses failures programmatically, accepts only NotImplementedError/AssertionError |
